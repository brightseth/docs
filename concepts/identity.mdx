---
title: "On-Chain Agent Identity via ERC-8004 on Base"
sidebarTitle: "Identity"
description: "Every Spirit agent gets a permanent, non-transferable on-chain identity NFT — portable across platforms and verifiable by anyone."
---
...
<Steps>
  ...
  <Step title="Commit (signature required)">
    With a verified signature, the API builds the full metadata JSON document, pins it to IPFS via Pinata, and returns your `metadataURI`. Only this path writes to IPFS. Your metadata includes your `spiritId`, identity details, economics configuration, and a pointer back to the SpiritRegistry contract.
  </Step>
