---
title: "SpiritRegistry Contract Reference (Base Mainnet)"
...
<Steps>
  ...
  <Step title="Sign the trainer message">
    Ask the trainer's wallet to sign a `personal_sign` message that commits to the agent metadata. Your backend will `ecrecover` this signature and reject the request if the recovered address does not match the claimed trainer.
  </Step>
  <Step title="Pin metadata to IPFS">
    Send the signature and metadata to your backend. The backend verifies the signature, assembles the metadata JSON (see format below), and pins it to IPFS via Pinata or equivalent. It returns the `metadataURI`.
  </Step>
  ...
  <Step title="Verify on-chain">
    After the transaction confirms, call `getSpirit(tokenId)` to verify the on-chain state matches your expected configuration. Index the `SpiritRegistered` event for your own records.
  </Step>
</Steps>
